...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        Returns:
            ...
            Ex:
                ...
                Gradient to a and b
        """
        # Let the gradients be zeros initially
        grad_1 = np.zeros(self.arr.shape)
        grad_2 = np.zeros(self.arr.shape)
        # If operand 1 requires gradient
        if(self.history[1].requires_grad == True):
            # if the gradients is not none then apply chain rule
            if(gradients is not None): grad_1 = gradients
            # otherwise, gradient will be ones of size = arrays shape
            else: grad_1 = np.ones(self.arr.shape)
        # If operand 2 requires gradient
        if(self.history[2].requires_grad == True):
            # if the gradients is not none then apply chain rule
            if(gradients is not None): grad_2 = gradients
            # otherwise, gradient will be ones of size = arrays shape
            else: grad_2 = np.ones(self.arr.shape)
    ...
    def grad_matmul(self, gradients=None):
        ...
        """
        # Let the gradients be zeros initially
        grad1 = np.zeros(self.arr.shape)
        grad2 = np.zeros(self.arr.shape)
        # if gradients is None
        if(gradients is None): gradients = np.ones(self.arr.shape)
        # Calculate using chain rule
        # If operand 1 requires gradient, calculate gradient for a = G@bT
        if(self.history[1].requires_grad == True): grad1 = (gradients) @ (np.transpose(self.history[2].arr))
        # If operand 2 requires gradient, calculate gradient for b = aT@G
        if(self.history[2].requires_grad == True): grad2 = (np.transpose((self.history[1].arr))) @ (gradients)
        # return tuple of gradients
        return (grad1, grad2)
    ...
    def backward(self, gradients=None):
        ...
        """
        # Recursively traverse through the DAG until leaf node.
        # If operation is matmul
        if(self.history[0] == 'matmul'):
            # Calculate the gradients for matmul operation
            result_matmul = self.grad_matmul(gradients)
            # Call backward on the parent nodes
            self.history[1].backward(result_matmul[0])
            self.history[2].backward(result_matmul[1])
        # If operation is add
        elif(self.history[0] == 'add'):
            # Calculate the gradients for add operation
            result_add = self.grad_add(gradients)
            # Call backward on the parent nodes
            self.history[1].backward(result_add[0])
            self.history[2].backward(result_add[1])
        # If leaf node is found
        else:
            # Cumulate it to take care of same node operations
            self.grad = self.grad + gradients